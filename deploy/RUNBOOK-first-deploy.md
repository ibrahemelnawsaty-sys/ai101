...
`email_verified_at` يُضبط لحظة الإنشاء **عمدًا**: الحساب لمن يشغّل الطرفية على الخادم نفسه،
ومسار إعادة إرسال التفعيل يرفض حسابًا موثَّقًا أصلًا.

للمدربين: `--role=trainer`، ثم يُسنَد المدرب إلى دفعته من شاشة **الدفعات** ببريده.
**المتدربون لا يُنشؤون من الطرفية إطلاقًا** — الأمر يرفض `--role=participant` قبل أن يسأل شيئًا
(D-69). يُضافون من لوحة الإدارة: **«أضف مستخدمًا»** لشخص واحد أو **«استيراد جماعي»** لقائمة؛
وكلاهما يُنشئ الحساب ويُجلسه في الدفعة ويرسل الدعوة بكلمة مؤقتة (D-63).

> **فحص لما قبل هذا التعديل:** نسخ سابقة من هذا الدليل كانت تطلب `--role=participant`. حساب
> متدرب أُنشئ هكذا لا ينتمي إلى أي دفعة، ولا شاشة اليوم تُجلسه لاحقًا. للعثور على أي منها:
> ```sh
> php artisan tinker --execute="echo App\Models\User::where('role','participant')->whereDoesntHave('enrollments')->pluck('email')->implode(PHP_EOL);"
> ```
...
|---|---|
| `D-02` البريد | **محسوم** (9 سبتمبر 2026): SMTP من صندوق المركز `info@athar-dev.edu.sa` على Google Workspace. كلمة مرور التطبيق في `.env` على الخادم وحده، ولا تُكتب في المستودع. |
