...
من hPanel ← Advanced ← Cron Jobs. الأسطر الكاملة في **`deploy/cron-setup.txt`**، وفيها أربعة
مدخلات. **استبدل `/usr/bin/php` بالمسار المطلق من المرحلة 1، و`/home/u123456789` بمسار `$HOME`
الحقيقي، في كل سطر.**
...
`schedule:work` · مدخل `schedule:run` ثانٍ · أي مدخل بلا `>> <ملف> 2>&1`.
