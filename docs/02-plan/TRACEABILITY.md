...
| BR-10 | التعديل اليدوي على الحضور يتطلب سببًا ويُسجَّل في سجل التدقيق | 🟩 | `AttendanceRecorder.php` · `app/Services/Audit/AuditLogger.php` · `app/Http/Requests/Trainer/UpdateAttendanceRequest.php` | `AttendanceRulesTest.php` — `it('BR-10: سجل التدقيق يحفظ القيمة قبل التعديل وبعده')` · اللوحة تحفظ صفًّا غير مكتمل وحقولها كلها يعرفها الطلب · لا زرّ تعديل بلا سجلّ · التحضير الجماعي لمن لا سجلّ له ولا يكتب لمنسحب · `markWithoutRecord` يحفظ ما قبله (D-72) |
...
| BR-19 | إعادة التسليم تحفظ الإصدار السابق ولا تحذفه | 🟩 | `app/Models/Submission.php` · `ScoreCalculator.php` · `database/migrations/2026_01_01_000005_create_assignments_and_submissions_tables.php` | `AssignmentAndProjectRulesTest.php` — `it('BR-19: رقم الإصدار يزيد ولا يعود للخلف أبدًا')` · `Screens/TrainerProfileLinksTest.php` — ملف المتدرّب يقرن كل مهمة بأحدث نسخها (D-72) |
...
| BR-22 | المتدرب لا يصل إلى بيانات متدرب آخر | 🟩 | `app/Services/Permissions/RoleResolver.php` · `CohortScope.php` · `app/Http/Middleware/EnsureCohortScope.php` · `app/Policies/Concerns/InteractsWithScope.php` · كل `app/Policies/*` | `tests/Feature/Authorization/CrossTenantAccessTest.php` — `it('BR-22: المتدرب لا يصل إلى تسليم متدرب آخر بتغيير المعرّف في الرابط')` · `RouteAuthorizationTest.php` · `AccessRulesTest.php` · `Screens/FileDownloadsTest.php` — المتدرّب يفتح ملفّه ولا يفتح ملف زميله ولو حمل رابطًا موقَّعًا (D-80) · `DirectConversationsTest.php` — `it('BR-22 (D-118): 403 — لا المشرف العام ولا مدرب آخر في الدفعة ولا المنسق ولا مدير النظام يفتح محادثة غيره')` وكل أزواج قاعدة بدء المحادثة (`ConversationRules` · `Thread::scopeVisibleTo` · `ThreadPolicy`) |
| BR-23 | المدرب لا يصل إلا لبيانات دفعاته | 🟩 | `RoleResolver.php` · `EnsureCohortScope.php` · `EnsureRole.php` · `app/Http/Requests/Trainer/Concerns/ScopedToCohort.php` · `app/Http/Controllers/Concerns/ReadsCohortScope.php` | `CrossTenantAccessTest.php` — `it('BR-23: المدرب لا يرصد درجة لمتدرب في دفعة أخرى')` · `tests/Feature/BusinessRules/AccessRulesTest.php` · `Admin/CohortTrainerAssignmentTest.php` — نموذج الإسناد يرسل ما يطلبه الطلب · الإسناد بالبريد · رفض بريد المتدرّب ظاهرًا على الحقل · المصفوفة لا تُسقط الصفحة · المدرّب المُزال لا يبقى مدرجًا (D-69) — تفشل على الشيفرة السابقة · `Screens/FileDownloadsTest.php` — رابط موقَّع لا يفتح ملفًّا لمدرّب دفعة أخرى (D-80) |
| BR-24 | رابط الزوم لا يُكشف قبل 15 دقيقة من البداية | 🟩 | `app/Models/Session.php` · `app/Policies/SessionPolicy.php` · `app/Presenters/Participant/SessionPresenter.php` · `NextSessionPresenter.php` | `AccessRulesTest.php` — `it('BR-24: الرابط يُسلَّم عند الحد بالضبط قبل الجلسة بخمس عشرة دقيقة')` · `it('BR-24: رابط الزوم لا يظهر في شيفرة الصفحة قبل الجلسة بخمس عشرة دقيقة')` · `Screens/DashboardNextSessionTest.php` — البطاقة وصفحة البثّ ونقطة الدخول تتّفق عند S-45m±1ث وE±1ث · النافذة رقم المدرّب للجلسة (D-52) لا 15 ثابتة، و`LiveController::joinWindowOpen()` قاعدتها الوحيدة (D-75) |
| BR-25 | صفحة التحقق لا تعرض بيانات شخصية حساسة | 🟩 | `app/Models/DigitalCard.php` · `Certificate.php` · `app/Http/Controllers/Public/CardVerificationController.php` · `CertificateVerificationController.php` · `resources/views/public/card-verify.blade.php` · `certificate-verify.blade.php` | `tests/Feature/BusinessRules/CertificateRulesTest.php` — `it('BR-25: صفحة التحقق من الشهادة تعرض ما نصّت عليه الوثيقة ولا شيء غيره')` · `tests/Feature/Screens/PublicScreensTest.php` · `SerialNumberGeneratorTest.php` · `BusinessRules/NotificationEventsTest.php` — الإصدار يُعلن برقمه المحفوظ ورابط تحقّقه، وزرّ الرسالة يفتح صفحة 200 (D-77) |
| BR-26 | الشهادة تُصدر باستيفاء نسبة الحضور ودرجة النجاح **معًا** | 🟦 | `app/Services/Certificates/CertificateEligibility.php` · `SerialNumberGenerator.php` · `app/Support/AttendanceCounting.php` · `app/Http/Controllers/Admin/CertificateController.php` | `tests/Unit/Services/CertificateEligibilityTest.php` — `it('BR-26: حد نسبة الحضور يُقرأ من الدفعة ويُختبر عند الحد بالضبط')` · `CertificateRulesTest.php` — `it('BR-26: حضور كامل ودرجة راسبة لا يمنحان الشهادة')` · `BusinessRules/NotificationEventsTest.php` — تنبيه النزول تحت الحد PRD §9.16.1: مرّة عبر تشغيلات · لا شيء قبل انتهاء جلسة (عند النهاية ±1ث) · عند الحد تمامًا لا · التعافي يعيد التسليح · رفع الحد يُنبِّه · النسبة كما في الشاشة · لا للمنسحب ولا لدفعة غير جارية · نصّ آخر جلسة · المسوِّي يستدعيه (D-77) |
...
| BR-31 | كل محتوى قابل للتغيير يُدار من لوحة الإدارة ولا يُكتب في الكود | 🟩 | `app/Models/LandingSetting.php` · `LandingContent.php` · `Program.php` · `Cohort.php` · `app/Http/Controllers/Admin/LandingController.php` · `app/Services/Landing/{LandingCatalog,LandingOverrides,LandingContentLoader}.php` · `app/Http/Requests/Admin/{Publish,Preview,Reset}LandingRequest.php` · `app/Presenters/Admin/LandingEditor.php` · `SettingController.php` · `database/seeders/SeedContent.php` · `database/migrations/2026_09_24_100000_create_landing_contents_table.php` (`D-114`) | `tests/Feature/BusinessRules/AdminRulesTest.php` — `it('BR-31: نصوص صفحة الهبوط تُقرأ من قاعدة البيانات وتتغير من لوحة الإدارة')` · `PublicScreensTest.php` · `tests/Feature/Admin/LandingContentEditorTest.php` — `it('BR-31: نص يُنشر من المحرّر يحلّ محلّ نص الملف في صفحة الهبوط')` · `it('BR-31: نص يُسقط قيمة حيّة مثل :count يُرفض ولا يُحفظ')` · `it('BR-31: العربية والإنجليزية تُحرَّران معًا أو كلٌّ على حدة')` · `it('BR-31: المعاينة تعرض الصفحة الحقيقية بالمسودة ولا تحفظ منها شيئًا')` · `it('BR-31: إرجاع قسم يحذف تعديلات نصوصه وحده ويُسجَّل في سجل التدقيق')` · `tests/Unit/Services/LandingContentTest.php` · **`D-117` — المحرّر لمدير النظام وحده:** `AdminRulesTest.php` — `it('BR-31: المتدرب والمدرب والمشرف العام لا يعدّلون إعدادات صفحة الهبوط')` · `SystemAdminRoleTest.php` — مدير النظام يحرّر، والمشرف 403 ويُسجَّل الرفض · `RegistrationIntakeTest.php` — `it('D-117: المشرف العام يغلق التسجيل ثم يفتحه، والنموذج يطيعه في الطلب التالي، ويُسجَّل كل تغيير')` وستة غيره (المفتاح عند المشرف العام: `RegistrationController::intake` · `SetRegistrationIntakeRequest` · `LandingSetting::switchIsOn()` · `Cohort::acceptsRegistrations()`) · `LandingContentEditorTest.php` — `it('D-117: المحرّر لا يُغلق التسجيل — يُرفض الطلب برسالة تدلّ على المشرف العام، والصفحة تبقى مفتوحة')` |
| BR-32 | يبقى مدير نظام فعّال واحد على الأقل دائمًا — **وبعد `D-117`: مشرف عام فعّال واحد على الأقل ومدير نظام فعّال واحد على الأقل** | 🟩 | `app/Services/Permissions/RoleResolver.php` (`isLastActiveHolder()` — المصدر الوحيد للقاعدة) · `RoleGate.php` · `app/Policies/UserPolicy.php` · `app/Http/Controllers/Admin/UserController.php` · `app/Console/Commands/ChangeRole.php` · `app/Http/Requests/Admin/ChangeUserStatusRequest.php` · `ChangeUserRoleRequest.php` | `AdminRulesTest.php` — `it('BR-32: حذف آخر مشرف عام فعّال مرفوض')` · `it('BR-32: مدير النظام لا يحذف حسابه ولا يعطّله ولا يغيّر دوره')` · `it('BR-32: تعطيل آخر مشرف عام فعّال مرفوض')` · `it('BR-32: تنزيل دور آخر مشرف عام إلى مدرب مرفوض')` · `it('BR-32: مع وجود مشرفَين عامَّين يمكن تعطيل أحدهما وتغيير دوره')` · `it('BR-32: آخر مدير نظام فعّال لا يُنزَّل دوره ولو من الطرفية')` · `it('BR-32: حذف المستخدم حذف ناعم لا فعلي')` · `DemoAccountsTest.php` — خطوتا ما بعد النشر (`D-117`) · `SystemAdminIntegrityTest.php` — `it('BR-32: إزالة الحسابات التجريبية تُبقي آخر مشرف عام وآخر مدير نظام، وتسجّل كل حذف')` · `it('BR-32: الفحص المقفل يعطي الجواب نفسه — آخر حامل للدور، ومع وجود غيره')` · `SystemAdminIntegrityTest.php` — `it('BR-32 (D-119): حساب مدعوّ لم يقبل دعوته لا يُحسب — فلا يُعطَّل آخر حامل فعلي للدور، ويُسمح بعد قبول الدعوة')` |
| BR-33 | وضع المعاينة: قراءة فقط صارمة مفروضة في طبقة البيانات | 🟩 | `app/Services/Permissions/ImpersonationService.php` · `app/Http/Middleware/ImpersonationReadOnly.php` · `BlockWhenImpersonating.php` · `app/Support/ImpersonationContext.php` | `tests/Feature/BusinessRules/ImpersonationRulesTest.php` — `it('BR-33: تسجيل الحضور أثناء المعاينة مرفوض على الخادم')` · `it('BR-33: انتهاء المعاينة تلقائيًا بعد ثلاثين دقيقة')` · `it('BR-33: مدير النظام يعاين حساب المشرف العام قراءةً فقط — يرى لوحته ولا يكتب فيها (D-117)')` · `SystemAdminRoleTest.php` — المعاينة نموذج POST لا رابط (كان يُجاب 405) · انتهاؤها يعيد إلى قائمة المستخدمين (`D-117`) · `SystemAdminPreviewTest.php` — `it('BR-33: أثناء معاينة مدرب يُرفض كل تصدير وتنزيل ورمز التحضير 403، ويُسجَّل كل رفض')` وأخواتها (المتدرب، المشرف العام، الشاشات بلا زر تصدير) · `SystemAdminIntegrityTest.php` — كل نهاية معاينة تُكتب (الخروج، تعطيل المُعايَن، انتهاء المدة) · `RegistrationIntakeTest.php` — `it('BR-33: معاينة حساب المشرف العام لا تحرّك مفتاح التسجيل — 403')` |
| BR-34 | المعاينة لا تغيّر أي أثر: آخر دخول · قراءة الإشعارات والرسائل · عدّادات التحميل | 🟩 | `ImpersonationReadOnly.php` · `ImpersonationContext.php` · `app/Models/Notification.php` · `ThreadParticipant.php` · `Resource.php` | `ImpersonationRulesTest.php` — `it('BR-34: المعاينة لا تزيد عدّاد تحميل الموارد')` · `it('BR-34: التصفح العادي خارج المعاينة يترك أثره كالمعتاد')` · `DirectConversationsTest.php` — `it('BR-33 (D-118): المعاينة تعرض محادثات الحساب المُعايَن، ولا تبدأ محادثة ولا تعلّم شيئًا مقروءًا')` |
| BR-35 | كل جلسة معاينة تُسجَّل، ولا تُعايَن حسابات المديرين — **بعد `D-117`: يعاين مدير النظام وحده أي حساب إلا حساب مدير نظام آخر** | 🟩 | `ImpersonationService.php` · `AuditLogger.php` · `app/Policies/UserPolicy.php` · `app/Models/ImpersonationSession.php` | `ImpersonationRulesTest.php` — `it('BR-35: لا يمكن معاينة حساب مدير نظام آخر')` · `it('BR-35: المشرف العام لا يملك صلاحية المعاينة (D-117)')` · `SystemAdminRoleTest.php` — `it('BR-35: خدمة المعاينة تعيد فحص القواعد بنفسها — لا تثق بالسياسة وحدها — وتسجّل كل رفض')` · `RouteAuthorizationTest.php` · `SystemAdminPreviewTest.php` — `it('BR-35: لا معاينة لحساب معلّق ولا غير مفعّل ولا لنفسك ولا لمدير نظام آخر — 403، والسبب ظاهر في صفحته')` · `it('BR-35: الحساب المحذوف يُرفض بسببه — لا يُعاين ولا يُفتح له سجل معاينة')` (`ImpersonationService::refusalReason()` — المصدر الواحد للسبب) |
...
| SCR-9.5 | هيكل لوحة التحكم (الجانبية · الهيدر · الرئيسية) | 🟦 | `components/layout/sidebar.blade.php` · `header.blade.php` · `footer.blade.php` · `layouts/app.blade.php` · `participant/dashboard.blade.php` | `ParticipantScreensTest.php` — `it('كل شاشات المتدرب تحمل وسم الشاشة واتجاه الصفحة من اليمين')` · `DashboardIsolationTest.php` — مفاتيح الخطأ التسعة متطابقة بين القالب والمتحكّم · بطاقة ترمي تعرض خطأها وحدها والصفحة 200 ولا تُسجَّل رسالة الاستثناء (D-66) — تفشل على الشيفرة السابقة · ⬜ **بنية قائمة المدرب والمدير مفتوحة على `D-30`** · `Screens/AppShellComposerTest.php` — التذييل يسمّي الدفعة · كل خيار في المبدّل مقبول · شارة المهام عند الموعد ±1ث · الجرس لهذا الحساب وحده · شارة الرسائل بلا رسائل القارئ وتُصفَّر بالفتح · المعاينة بلا مبدّل ولا علامة قراءة · قاعدة الدور الواحدة · عدٌّ يرمي لا يُسقط الصفحة (D-75) · `Screens/DashboardNextSessionTest.php` — نافذة المدرّب على البطاقة (D-75) · `Screens/CrossDeviceContractTest.php` — خطّ واحد وبريد بالخطّ نفسه · صندوق التمرير يحتوي ما فيه · عمود المحتوى لا يتجاوز الشاشة · قائمة المحادثات على الجوّال · قائمة الحساب داخل الشاشة · أسماء روابط الشريط المطويّ · 44/16px على اللمس · كل dvh بعد vh · طباعة الشهادة بلا :has() وحده · كلمة مرور الاجتماع عند نافذة الدخول ±1ث (BR-24) · البطاقة للمتدرّب وحده · بلا جافاسكربت (D-86) · `tools/offline-checks/measure-responsive.mjs` — 44 شاشة × 8 عروض في متصفح حقيقي: صفر عطل مانع (D-86) · `Screens/RailLayoutContractTest.php` — شريط كل دور (المتدرب · المدرب · المنسّق · المشرف) ثلاث مناطق مباشرة بالترتيب في الشريط والدرج · قائمة الروابط وحدها تتمرّر ولها أرضية · الاسم مخفي بصريًا لا محذوف في الشريط المطويّ · هامش التذييل الجانبي · لا صنف `u-*` بلا قاعدة · إظهار الرابط الحالي لا يحرّك الصفحة — الستة تفشل على الشيفرة السابقة (D-115) · `tools/offline-checks/measure-rail-fit.mjs` — 43 شاشة × 7 ارتفاعات × مفتوح/مطويّ = 602 تخطيط في متصفح حقيقي: صفر تداخل وصفر رابط أو زر يتعذّر بلوغه وصفر حلقة تركيز مقصوصة، وكانت الـ602 كلها تفشل قبل الإصلاح (D-115) |
...
| SCR-9.9 | الحضور والغياب | 🟩 | `participant/attendance.blade.php` · `trainer/attendance.blade.php` · `app/Http/Controllers/Participant/AttendanceController.php` · `app/Presenters/Participant/AttendanceRatePresenter.php` | `ParticipantScreensTest.php` (ثلاث حالات) · `AttendanceRulesTest.php` · `AttendanceWindowTest.php` · `Screens/TrainerRosterLiveTest.php` — الكشف الحيّ PRD §9.9.7: خانات الصفحة = خانات النقطة · الدخول يظهر في التحديث التالي · BR-23 · `measure-alpine-pages.mjs` — بمتصفح: الخانات تتحدّث ومربّع محدَّد يبقى (D-72) · `Screens/CheckinCodeCardTest.php` — بطاقة رمز التحضير الذاتي تصل المتصفح بتعبير مترجَم للمدرب والمنسّق والمشرف · تحمل قرار الخادم عند S-1s وS وS+1s وS+60m-1s وS+60m وS+60m+1s · لا توجيه Blade غير @class و@style داخل سمات وسم مكوّن في أي قالب — الثلاثة تفشل على الشيفرة السابقة (D-116) |
...
| SCR-9.13 | نظام التواصل الداخلي | 🟦 | `participant/messages.blade.php` · `app/Http/Controllers/Participant/MessageController.php` · `app/Policies/ThreadPolicy.php` | `ParticipantScreensTest.php` — `it('شاشة الرسائل: الحالة الفارغة حين لا محادثات')` · `it('شاشة الرسائل: الحالة العادية بعد وجود محادثة')` · ⬜ هيكل التحميل وحالة الخطأ غير مختبَرين · `Screens/MessageLiveUpdateTest.php` — الاستطلاع يعيد القائمة مُصيَّرة ومُهرَّبة · `latest` لا يتغيّر إلا بوصول رسالة · BR-22 على الاستطلاع (D-67: التحديث الحيّ PRD §9.13.2 منفَّذ لأول مرة) · `Screens/MessagingProvisionTest.php` — المدعوّ يُمنح محادثاته الثلاث · إسناد المدرّب يمنحه القناتين ومحادثة مع كل متدرّب بلا تكرار · أمر التعبئة لا يتكرّر أثره · المدرّب ينشر إعلانًا والمتدرّب لا · المحادثة المباشرة باسم الطرف الآخر (D-82) |
...
| SCR-9.16 | الإشعارات | 🟩 | `participant/notifications.blade.php` · `app/Http/Controllers/Participant/NotificationController.php` · `app/Models/NotificationPreference.php` | `ParticipantScreensTest.php` (ثلاث حالات) · `ImpersonationRulesTest.php` (BR-34) · ⬜ **قناة البريد معطَّلة بـ `D-02`** · `Mail/LetterContractTest.php` — التسميات وكتل النصّ وأسماء المسارات وتطابق ar/en وأقفال الجدولة (D-62) · ✅ **قناة البريد تعمل فعلًا — `D-02` محسوم** · ✅ **تفضيلات البريد يقرؤها كل مُرسِل غير أمني — `Mail/MailPreferencesTest.php` عبر المستمعين الحقيقيين (D-66)** · ⬜ **لا تذكير جلسة ولا تذكير تسليم (D-62)** · `Auth/InvitationFlowTest.php` + `Admin/UserImportTest.php` — الدعوة والإلحاق وكلمة المرور المؤقتة والتغيير الإجباري والاستيراد (D-63) — ✅ **نُفِّذت (12 سبتمبر): 9/9 — وكشف التنفيذ الأول أن حالة BR-29 لم تُفعِّل مشغّل الجلسات `database` كما يطلب `phpunit.xml` فأُصلحت، وتفشل الآن إن حُذف الإبطال** · `Screens/SmallContractsTest.php` — جدول التفضيلات يُبنى من كتالوج الأنواع بجمهورها في PRD §9.16.1، ومتدرّب حقيقي بلا صفوف يراه كاملًا، والنموذج كما يُصيَّر يقبله طلبه · المفتاح المعطَّل يُرسل حالته (D-78) · `Auth/InvitationLinkTest.php` — الدعوة باسم وبريد فقط · الرابط يفتح النموذج والبريد معروض لا مُرسَل · القبول يفعّل ويُدخل ويستهلك الرابط · بريد مُرسَل مع النموذج يُتجاهل · رابط منتهٍ أو مستعمل أو مجهول بالنصّ نفسه · إعادة الإرسال تُبطل الأول · الاستعادة لمن دُعي تُرسل دعوته · `Admin/UserImportTest.php` — الملف ثلاثة أعمدة واسم ثنائي يُقبل (D-85) · `Admin/BroadcastTest.php` — شاشة «مراسلة المتدربين»: الرسالة لمتدربي الدفعة النشطين وحدهم وبتفضيلاتهم · الضغط المزدوج · النص مُهرَّب · التذكير اليدوي بالجلسات في رسالة واحدة ولا يمسّ التلقائي · المهام لكل متدرب بما لم يسلّمه وحده · المهلة ±1ث · الصلاحيات (D-87) |
...
| SCR-9.18 | لوحة مدير النظام | 🟦 | `admin/dashboard.blade.php` · `programs.blade.php` · `cohorts.blade.php` · `users/index.blade.php` · `show.blade.php` · `audit.blade.php` · `settings.blade.php` · `registrations.blade.php` · `reports.blade.php` · `landing.blade.php` («محتوى صفحة الهبوط» — محرّر كل نصوص الصفحة بلغتيها بمعاينة حيّة للحاسوب والجوال، `D-114`) · `admin/partials/landing-*.blade.php` · `resources/js/landing-editor.js` | `TrainerAdminScreensTest.php` — `it('لوحة المدير: الحالة العادية تعرض البطاقات الإحصائية')` · `AdminRulesTest.php` · **`D-117` — اللوحة صارت لدورين:** المشرف العام (كل الشاشات عدا المستخدمين وصفحة الهبوط) ومدير النظام (المستخدمون وصفحة الهبوط والمعاينة وحدها) — `SystemAdminRoleTest.php` (17 اختبارًا) · `SidebarRailTest.php` — `it('D-117: مدير النظام يرى المستخدمين وصفحة الهبوط وحدهما')` · `AdminAccountRecordsTest.php` — إجلاس حساب قائم من شاشة الدفعات للمشرف، وصفحة الحساب بلا سجل تدقيق · `RouteAuthorizationTest.php` (عمود `system_admin`) · ⬜ **الحالات الفارغة وهياكل التحميل غير مختبَرة لأغلب شاشات المدير** |
...
| BR-19 | التسليم لا يُستبدَل بل تزيد نسخته | 🟨 | العدّ في `SubmissionController::stats` و`ReportController::submitterCounts` يستخدم `COUNT(DISTINCT …)` لا عدّ صفوف | اختبار يثبت أن ثلاث نسخ = تسليم واحد · `Screens/TrainerProfileLinksTest.php` — ملف المتدرّب يقرن كل مهمة بأحدث نسخها (D-72) |
...
| FR-NOTIF-10 | تذكير الجلسة قبل 24 ساعة وقبل ساعة — منصة + بريد — ولا يتكرّر مع إعادة تشغيل المهمة | 🟩 | `app/Services/Notifications/ScheduledNotices.php` · `app/Console/Commands/SendScheduledNotices.php` · `routes/console.php` (كل دقيقة) · ترحيل `create_scheduled_notices_table` | `BusinessRules/ScheduledNoticesTest.php` — حدود كل علامة ±1ث · S-24h مرة واحدة بالمدرّب و«غدًا» · S-1h و«مدربك» · ما فاتت مهلته يُترك · الملغاة والمنسحب والدفعة الأخرى والمنتهية · المنقولة تُذكَّر من جديد · كل قناة بمفتاحها · الجدولة كل دقيقة (D-83) · ⚠️ المهلة افتراض مؤقت |
| FR-NOTIF-11 | «بدأت الجلسة» عند S — منصة فقط — مرة واحدة | 🟩 | `app/Services/Notifications/ScheduledNotices.php` | `BusinessRules/ScheduledNoticesTest.php` — S-1ث لا · S نعم · لا بريد (D-83) |
| FR-NOTIF-12 | إلغاء **أو نقل** جلسة يُبلغ الدفعة على القناتين | 🟩 | `app/Http/Controllers/Trainer/SessionController.php` (`cancel()` · `update()`) · `app/Services/Notifications/CohortNotices.php` (`sessionMoved()`) · `app/Events/SessionRescheduled.php` · `app/Listeners/SendSessionRescheduled.php` | `BusinessRules/NotificationEventsTest.php` (الإلغاء، D-77) · `BusinessRules/CohortNoticesTest.php` — النقل يُبلغ بالموعد الجديد وتغيير الموضوع وحده صامت (D-83) |
| FR-NOTIF-14 | تذكير موعد المهمة قبل 48 وقبل 6 ساعات لمن لم يسلّم وحده | 🟩 | `app/Services/Notifications/ScheduledNotices.php` · `app/Listeners/SendAssignmentReminder.php` | `BusinessRules/ScheduledNoticesTest.php` — ±1ث عند كل علامة · من سلّم لا يصله · مرتان لا أكثر · المسودة صامتة (D-83) |
| FR-NOTIF-15 | تأكيد استلام التسليم للمتدرّب — منصة | 🟩 | `app/Services/Notifications/CohortNotices.php` (`handedIn()`) · `Participant/AssignmentController::submit` · `Participant/FinalProjectController::submit` | `BusinessRules/CohortNoticesTest.php` — المهمة والمشروع الختامي (D-83) |
| FR-NOTIF-16 | وصول تسليم جديد لمدرّب الدفعة وحده — منصة | 🟩 | `app/Services/Notifications/CohortNotices.php` · `app/Services/Mail/CohortAudience.php` (`trainerIds()`) | `BusinessRules/CohortNoticesTest.php` — مدرّب دفعة أخرى لا يصله · الرابط يفتح اللوحة مصفّاة (D-83) |
| FR-NOTIF-20 | مورد جديد للدفعة — منصة — قابل للتعطيل | 🟩 | `Trainer/ResourceController::store` · `app/Services/Notifications/CohortNotices.php` | `BusinessRules/CohortNoticesTest.php` — الجرس المطفأ لا يُكتب له (D-83) |
| FR-NOTIF-21 | إعلان جديد للدفعة — منصة + بريد | 🟩 | `Participant/MessageController::store` · `app/Services/Notifications/CohortNotices.php` (`posted()`) · `app/Events/AnnouncementPublished.php` · `app/Listeners/SendAnnouncementPublished.php` | `BusinessRules/CohortNoticesTest.php` — القناتان للمتدرّب، لا «رسالة جديدة» (D-83) |
| FR-NOTIF-22 | رسالة جديدة للمستقبِل — منصة، وبريد إن كان غير متصل | 🟩 | `app/Services/Notifications/CohortNotices.php` · `app/Listeners/SendMessageReceived.php` · `app/Services/Messages/Presence.php` · ترحيل `add_last_emailed_at_to_thread_participants` | `BusinessRules/CohortNoticesTest.php` — الاتصال عند 300ث لا بريد و301ث بريد · بريد واحد للمحادثة كل 15 دقيقة ±1ث (D-83) · ⚠️ القيمتان افتراض مؤقت (AMB-14) |
| FR-MSG-12 | كتم محادثة يمنع إشعاراتها وحدها | 🟦 | `app/Services/Notifications/CohortNotices.php` · `app/Listeners/SendMessageReceived.php` | `BusinessRules/CohortNoticesTest.php` — المكتومة لا جرس ولا بريد (D-83) · ⬜ **لا واجهة لضبط الكتم بعد** |
| FR-CERT-13 | لا يُحذف مستخدم له شهادة صادرة (ولا سجلات تقييم — PRD §7.8)، والحذف ناعم | 🟩 | `app/Policies/UserPolicy.php` (`delete()` · `holdsAcademicRecords()`) | `Screens/AdminAccountRecordsTest.php` — شهادة مسحوبة وتقييم يُرفضان 403 ومن ليس له يُحذف ناعمًا (D-84) · `Authorization/RouteAuthorizationTest.php` |
